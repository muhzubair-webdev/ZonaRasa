  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Kelola pesanan yang masuk.</p>
      </div>
          ...
          orders.map((order) => (
            <Card key={order.id} className={order.status === 'completed' ? 'border-green-500/50' : order.status === 'cancelled' ? 'border-destructive/50' : 'border-primary/50'}>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div>
                    <CardTitle className="text-xl flex items-center gap-2">
                      {order.customer_name}
                      {order.status === 'pending' && <Badge variant="secondary" className="bg-yellow-500/10 text-yellow-600 border-yellow-500/20"><Clock className="w-3 h-3 mr-1" /> Menunggu</Badge>}
                      {order.status === 'completed' && <Badge variant="secondary" className="bg-green-500/10 text-green-600 border-green-500/20"><CheckCircle className="w-3 h-3 mr-1" /> Selesai</Badge>}
                      {order.status === 'cancelled' && <Badge variant="destructive"><XCircle className="w-3 h-3 mr-1" /> Dibatalkan</Badge>}
                    </CardTitle>
              ...
              <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <h4 className="font-semibold mb-2 text-sm uppercase tracking-wider text-muted-foreground">Item Pesanan</h4>
                ...
                <div className="space-y-6">
                  <div>
                    <h4 className="font-semibold mb-1 text-sm uppercase tracking-wider text-muted-foreground">Lokasi Pengiriman</h4>
                    <p className="text-sm bg-muted/50 p-3 rounded-md border">{order.location_detail}</p>
                  </div>
                  {order.notes && (
                    <div>
                      <h4 className="font-semibold mb-1 text-sm uppercase tracking-wider text-muted-foreground">Catatan</h4>
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {order.status !== 'completed' && (
                      <Button onClick={() => updateStatus(order.id, 'completed')} className="flex-1 bg-green-600 hover:bg-green-700 text-white min-w-[140px]">
                        <CheckCircle className="w-4 h-4 mr-2" /> Selesaikan Pesanan
                      </Button>
                    )}
                    ...
                    {order.status !== 'pending' && (
                      <Button onClick={() => updateStatus(order.id, 'pending')} variant="outline" className="flex-1 text-yellow-600 hover:bg-yellow-500/10 border-yellow-500/20 min-w-[140px]">
                        <Clock className="w-4 h-4 mr-2" /> Kembalikan ke Menunggu
                      </Button>
                    )}
                  </div>
                </div>
